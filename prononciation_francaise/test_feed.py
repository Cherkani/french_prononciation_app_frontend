...
def process_audio():
    ...
    try:
        ...
        print(f"Sending audio file {audio_file} to transcription service")
        ...
        if not recognized_text:
            print("No recognized text from transcription.")
            return jsonify({"error": "No recognized text from transcription."}), 400

        # Get feedback from the feedback service
        print(f"Sending recognized text to feedback service: {recognized_text}")
        ...
        if feedback_response.status_code != 200:
            print(f"Feedback failed with status {feedback_response.status_code}")
